...
local EMERGENCE_BREAK_DENOMINATOR = 12
...
-- Goal: Red Seal Holographic Steel King of Hearts
...
local function apply_metamorphosis_upgrade(card)
    ...
    elseif upgrade == "VALUE" then
        ...
        (card_value_current == 'J' and 'Q') or
        (card_value_current == 'Q' and 'K')
...
local function joker_emergence(self, context)
    ...
    if context.cardarea == G.jokers and context.after and context.scoring_hand then
        ...
        for i, card in ipairs(emergenceCards) do
            if not card.destroyed and not card.shattered then
                card_eval_status_text(card, 'extra', nil, nil, nil, {message = "Evolved!"})
    ...
    elseif context.destroying_card then
        ...
        if not card_is_rshskoh(card) and pseudorandom('177013') < G.GAME.probabilities.normal/EMERGENCE_BREAK_DENOMINATOR then
            card_eval_status_text(context.destroying_card, 'extra', nil, nil, nil, {message = "Broken!"})
            return true
        end
        return
    end
...
function SMODS.INIT.SnoresvilleEmergence()
    ...
    for i, j in ipairs(joker_order_thing) do
        ...
        SMODS.Sprite:new(v.slug, SMODS.findModByID("SnoresvilleEmergence").path, "j_snoresville_emergence.png", 71, 95, "asset_atli")
        :register()
    end
...
local absolute = SMODS.Deck:new(DECK_NAME, "snoresvilleMetamorphosis", {polyglass = true}, {x = 0, y = 3}, loc_def)
absolute:register()